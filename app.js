...
async createProject(form) {
    ...
    showProjectInfo(projectId) {
        ...
        modalContent.innerHTML = `
            <div class="project-loading">
                <i class="fas fa-spinner fa-pulse"></i> Loading project information...
            </div>
        `;
        ...
        // Fetch project information from Google Sheets
        this.fetchProjectInfo(project.name)
            .then(allotmentData => {
                // Format and display the data
                this.displayProjectInfo(project, allotmentData);
            })
            .catch(error => {
                modalContent.innerHTML = `
                    <div class="error-state">
                        <i class="fas fa-exclamation-triangle"></i>
                        <h4>Error loading project information</h4>
                        <p>${error.message}</p>
                    </div>
                `;
            });
    ...
    fetchProjectInfo(projectName) {
        // This would normally be a fetch to a backend API that accesses the Google Sheet
        // For this example, we'll simulate with the data we've seen
        return new Promise((resolve, reject) => {
            // Simulated data from the Google Sheet
            const allotmentData = [
                {
                    "Project Name": "Market Expansion Analysis for Southeast Asia",
                    "Description": "Conduct a comprehensive market expansion analysis for Thailand, Vietnam, and Indonesia. Includes competitor benchmarking, consumer behavior studies, pricing strategy formulation, and development of a go-to-market plan. Requires coordination between Marketing, Sales, Finance, and Research teams. Final deliverable includes a feasibility report and regional strategy.",
                    "Due Date": "2025-08-30",
                    "Required Skills": "Market Research, Data Analysis, Financial Forecasting, Strategic Planning, Report Writing, Cross-Cultural Marketing",
                    "Status": "Not Started"
                },
                {
                    "Project Name": "Internal HR System Automation using Python",
                    "Description": "Develop an internal tool for automating HR processes like candidate tracking, onboarding, and performance tracking. The system will be built using Python (Flask), with integration to Zoho and Google Workspace. Must ensure GDPR compliance and role-based access control. Involves IT and HR collaboration. Deliverables include a tested and deployed system with documentation.",
                    "Due Date": "2025-07-15",
                    "Required Skills": "Python, Flask, API Integration, UI/UX Design, HR Operations, GDPR Compliance",
                    "Status": "Not Started"
                },
                {
                    "Project Name": "Real-Time Sales Performance Dashboard",
                    "Description": "Build a live dashboard using Power BI and Python to track regional and global sales metrics. Integrates Salesforce, Excel, and CRM data via APIs. Includes conversion rate analysis, revenue trends, and rep KPIs with filterable views. Output should be device-responsive and deployable internally. Coordination with Sales, IT, and Finance is required.",
                    "Due Date": "2025-08-20",
                    "Required Skills": "Power BI, SQL, Python, Data Visualization, API Handling, KPI Design",
                    "Status": "Not Started"
                },
                {
                    "Project Name": "Employee Experience Survey & Action Plan",
                    "Description": "Design and analyze a company-wide employee survey on satisfaction and engagement. Use Google Forms/SurveyMonkey for collection and Excel/R for analysis. Based on results, develop department-specific action plans with workshops. Involves HR leadership and inter-departmental feedback sessions. Final deliverables include anonymized datasets, published findings, and actionable plans.",
                    "Due Date": "2025-07-10",
                    "Required Skills": "Survey Design, Data Analysis, Employee Engagement, Excel, Communication Strategy",
                    "Status": "Not Started"
                },
                {
                    "Project Name": "Engineering Design Optimization for Aero Components",
                    "Description": "Redesign aerospace components to improve weight-to-strength ratio using SolidWorks and ANSYS. Perform iterative simulations and validate designs via wind tunnel testing. Ensure compliance with ISO/aerospace standards. Deliverables include optimized CAD models, cost analysis, and technical documentation. Collaboration needed with Engineering and Finance departments.",
                    "Due Date": "2025-09-05",
                    "Required Skills": "CAD (SolidWorks), FEA (ANSYS), Mechanical Design, Aerospace Standards, Cost Estimation, Compliance Documentation",
                    "Status": "Not Started"
                },
                {
                    "Project Name": "AI Project Manager",
                    "Description": "When given a project and employees as input (through manually or through csv), it allocates the project by dividing it into daily tasks to team members and analyzes their performance daily and gives this report to the team leader",
                    "Due Date": "2025-08-11",
                    "Required Skills": "Javascript, project management, UI dev",
                    "Status": "Not Started"
                }
            ];

            // Find the matching project data
            const projectData = allotmentData.find(p => p["Project Name"] === projectName);

            if (projectData) {
                setTimeout(() => resolve(projectData), 800); // Simulate network delay
            } else {
                setTimeout(() => reject(new Error("Project information not found in database")), 800);
            }
        });
    }

    displayProjectInfo(project, allotmentData) {
        const modalContent = document.getElementById('project-info-content');
        if (!modalContent) return;

        // Create a comparison of local data vs Google Sheets data
        const requiredSkills = allotmentData["Required Skills"].split(", ");

        modalContent.innerHTML = `
            <div class="project-info-container">
                <div class="project-info-header">
                    <h2>${project.name}</h2>
                    <span class="status status--${this.getStatusClass(project.status)}">${this.formatStatus(project.status)}</span>
                </div>

                <div class="project-info-section">
                    <h4>Description</h4>
                    <p>${allotmentData.Description}</p>
                </div>
                ...
                <div class="project-info-section">
                    ...
                    <div class="project-timeline">
                        <div class="timeline-item">
                            <span class="meta-label">Due Date:</span>
                            <span class="meta-value">${this.formatDate(allotmentData["Due Date"])}</span>
                        </div>
                        <div class="timeline-item">
                ...
                <div class="project-info-section">
                    ...
                    <div class="team-members-list">
                        ${project.assignedTo ? project.assignedTo.map(member => `
                            <div class="team-member-item">
                                <div class="member-avatar">${this.getInitials(member)}</div>
                                <span>${member}</span>
                            </div>
                        `).join('') : `<p>No team members assigned yet</p>`}
                    </div>
                </div>

                <div class="project-info-actions">
                    <button class="btn btn-outline" onclick="app.closeModal('project-info-modal')">
                        Close
                    </button>
